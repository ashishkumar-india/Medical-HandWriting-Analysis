...
def load_image(path):
    ...
    # Static channel dimension helps Keras build model shapes
...
def prepare_example(path, label, training):
    ...
    # Convert label Tensor → Python string → char IDs
    def _encode_label(l):
        l = l.numpy().decode("utf-8")
        return np.array([char_to_id[c] for c in l], dtype=np.int32)

    label_ids = tf.py_function(func=_encode_label, inp=[label], Tout=tf.int32)
    label_len = tf.shape(label_ids)[0]
...
def build_model(vocab_size):
    inputs = keras.Input(shape=(48, None, 1), name="image")  # fixed height=48
       # adjust shape as per your data
    ...
    # ✅ Reduce dimension safely with Lambda
...
def ctc_loss(y_true, y_pred, input_len, label_len):
    # Convert RaggedTensor labels to SparseTensor
    if isinstance(y_true, tf.RaggedTensor):
        y_true = y_true.to_sparse()
    ...
    loss = tf.nn.ctc_loss(
        ...
        blank_index=vocab_size,   # last index = blank
    )
...
def train_step(batch):
    ...
    # batch["label"] is already a padded dense tensor with value -1
    y_true = tf.RaggedTensor.from_tensor(batch["label"], padding=-1)
